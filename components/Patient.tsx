...
const Patient = ({ name, id, date, gender, age, time, doctorName, colorIndex = 0, isEncounter = false, isActive }: Props) => {
    ...
    return (
        <div className="flex flex-col gap-3 m-4 sm:flex-row sm:items-center sm:gap-4">
            ...
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between sm:gap-4 w-full">
                <div>
                    ...
                    {isEncounter && <small className="text-neutral-500/80">Dr. {doctorName}</small>}
                </div>
                <div className="flex flex-col sm:items-end">
                    <p className="text-sm text-neutral-500">{date} {time}</p>
                    {!isEncounter && <small className="text-neutral-500/80">{gender}, {age} years</small>}
                    {isEncounter && (
                        <small className={clsx(
                            "mt-1 inline-flex px-2 py-1 rounded-full font-medium",
                            isActive
